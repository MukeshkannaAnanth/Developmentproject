  ...
  return (
    <Stack direction='row' justifyContent={el.incoming ? 'start' : 'end'}>
        <Box p={1.5} sx={{
                ...
                <Stack spacing={1}>
                    <img src={`${senderreceiverImg}/${el.img}`} alt={el.message} style={{maxHeight: 210 , borderRadius:'10px'}}/>
                    <Typography variant='body2' color={el.incoming ? theme.palette.text : '#fff'}>
                        {el.message}
                    </Typography>
                </Stack>
  ...
  return (
    <Stack direction='row' justifyContent={el.incoming ? 'start' : 'end'}>
        <Box p={1.5} sx={{
                ...
                <Stack spacing={1}>
                <ReactPlayer
        url='http://localhost:8001/assets/SndRcvdVideo/video-1705169024961.mp4'
...
export { TimeLine, TextMsg, MediaMsg, ReplyMsg, LinkMsg, DocMsg, Video }